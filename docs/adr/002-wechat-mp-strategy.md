...
- **尚未接入具体公众号**：WeWe RSS 需要一次微信读书扫码登录，属于运维步骤，未在本机自动化完成。
- **验收口径**: 方案、配置模板、compose 可选服务已就绪；首个号接入后只需在 `config/wechat.yaml` 填 feed URL 并 `collect --only-rss`。
- **若短期不部署 WeWe RSS**: 可用手动转发——把感兴趣的推文 URL 丢进待读队列（Lab 5 按 URL 抽取），不阻塞其它版面。
